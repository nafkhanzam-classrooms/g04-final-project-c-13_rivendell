...
Allows for dual-player instantiation with custom control schemes.
"""

from constants import (
    COLS, CELL_SIZE, PIECE_COLORS, PIECE_GLOW, WALL_KICKS, FALL_SPEEDS, BASE_SCORES
)
...
class PlayerState:
    ...
    def __init__(self, name, board_x, board_y, controls):
        """
        Initialize player state.

        Args:
            name: String identifying the player (e.g., "Player 1")
            board_x: X offset of the board grid
            board_y: Y offset of the board grid
            controls: Dict mapping actions to pygame key constants
        """
        self.name = name
    ...
    def get_fall_speed(self):
        """Get speed of gravity based on level."""
    ...
    def lock_piece(self, shared_sequence):
        ...
        # Spawn particles and line clear flashes
        for row in cleared:
    ...
    def update(self, dt, shared_sequence):
        """Update physics timers and active particle effects."""
        if self.game_over:
            return
